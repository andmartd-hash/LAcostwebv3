...
# ==========================================
st.set_page_config(page_title="LAcostWeb V11", layout="wide", page_icon="🏢")

st.markdown("""
    <style>
    .main { background-color: #f4f6f9; }
    .stButton>button { width: 100%; border-radius: 5px; height: 3em; }
    /* Hacemos los inputs más compactos visualmente */
    .stNumberInput input { padding: 0px 5px; height: auto; }
    .stTextInput input { padding: 0px 5px; height: auto; }
    /* Ajuste de etiquetas para ahorrar espacio vertical */
...
def add_line():
    st.session_state.lines.append({
        ...
        "unit_cost": 0.0, "gp_percent": 20 # Guardamos como entero (20%)
    })
...
with st.sidebar:
    ...
    st.caption("LAcostWeb V11 | Compact Grid")

# --- MAIN ---
st.title("LAcostWeb V11 🚀")
...
# 2. LÍNEAS DE SERVICIO (GRID OPTIMIZADO)
...
for i, line in enumerate(st.session_state.lines):
    ...
    with st.expander(f"🔹 {i+1}. {current_offering_name}", expanded=True):
        ...
        # FILA 1: Catálogo y Borrar
        # [Oferta (Largo) | L40 | Conga | Delete]
        ...
        # FILA 2: Inputs Numéricos (Ajustado "Corto")
        # [Desc | Start | End | DUR | Qty | Cost | GP% | TOTAL]
        # Agregamos col_dur para ver la duración calculada
        r2_c1, r2_c2, r2_c3, r2_c_dur, r2_c4, r2_c5, r2_c6, r2_c7 = st.columns([1.5, 0.9, 0.9, 0.5, 0.6, 0.8, 0.6, 1.2])
        
        with r2_c1: 
            desc = st.text_input("Desc", key=f"desc_{i}", placeholder="Details...")
        with r2_c2: 
        ...
        with r2_c_dur:
            # CAMPO DURATION INCLUIDO
            st.text_input("Mth", value=str(dur), disabled=True, key=f"dur_disp_{i}")

        with r2_c4: 
            qty = st.number_input("Qty", min_value=1, value=1, key=f"qty_{i}")
        with r2_c5: 
            unit_cost = st.number_input("Cost ($)", min_value=0.0, key=f"uc_{i}")
        with r2_c6: 
            # GP EN VISTA PORCENTAJE (Entero 0-100)
            gp_percent_val = st.number_input("GP %", 0, 99, 20, step=1, key=f"gp_{i}")
            gp = gp_percent_val / 100.0 # Convertimos a decimal para fórmula
...
with col_sum2:
    ...
    if st.button("📧 Outlook Draft", type="primary", use_container_width=True):
        if not cust_name:
            ...
            subject = f"LAcostWeb V11: {cust_name} - Total {currency_sel} ${grand_total_price:,.2f}"
            body = f"""
