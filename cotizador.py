...
# ==========================================
st.set_page_config(page_title="LAcostWeb V14", layout="wide", page_icon="🏢")
...
# ==========================================
# 3. LAYOUT (V14 - Fixed Duration Calc)
# ==========================================
...
with st.sidebar:
    ...
    st.caption("LAcostWeb V14 | Duration Fix")
...
for i, line in enumerate(st.session_state.lines):
    ...
    with st.expander(f"🔹 {i+1}. {current_offering_name}", expanded=True):
        ...
        with r2_c2: 
            # Leemos el valor actual o el guardado
            s_s = st.date_input("Start", value=line.get("start_date", date.today()), key=f"ss_{i}")
            st.session_state.lines[i]["start_date"] = s_s # Actualizamos memoria
        with r2_c3: 
            s_e = st.date_input("End", value=line.get("end_date", date.today().replace(year=date.today().year + 1)), key=f"se_{i}")
            st.session_state.lines[i]["end_date"] = s_e # Actualizamos memoria
        
        # CÁLCULO INMEDIATO
        dur = calculate_months(s_s, s_e)
        
        with r2_c_dur:
            # FIX: Forzamos la actualización visual usando session_state directamente
