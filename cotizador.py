...
# ==========================================
# 0. CONFIGURACIÓN Y ESTILOS (IGUAL A V14)
# ==========================================
st.set_page_config(page_title="LAcostWeb V14 (Data V3)", layout="wide", page_icon="🏢")
...
# ==========================================
# 1. BASE DE DATOS (ESTRUCTURA V14 + DATOS V3)
# ==========================================

# DATOS DE PAÍSES (Actualizados V3)
...
# CATÁLOGO DE SERVICIOS (Completo 23 Items V3)
...
# ==========================================
# 3. LAYOUT (ESTRUCTURA V14 ESTRICTA)
# ==========================================
...
with st.sidebar:
    ...
    st.caption("LAcostWeb V14 (Data V3)")

# --- MAIN ---
st.title("LAcostWeb V14 🚀")

# 1. HEADER (Agregué los campos nuevos de V3 manualmente)
with st.container():
    # Nueva distribución para incluir ID_Cotizacion
    c_head1, c_head2, c_head3 = st.columns(3)
    with c_head1: 
        id_cot = st.text_input("ID Cotización", placeholder="COT-001")
        cust_name = st.text_input("Customer Name", placeholder="Client Name")
    with c_head2: 
        cust_num = st.text_input("Cust. #", placeholder="ID")
...
# 2. LÍNEAS DE SERVICIO (Lógica V14 corregida)
...
with col_sum2:
    ...
    if st.button("📧 Outlook Draft", type="primary", use_container_width=True):
        if not cust_name:
            ...
            subject = f"Quote {id_cot}: {cust_name} - {currency_sel} ${grand_total_price:,.2f}"
            body = f"""
ID: {id_cot}
