...
with st.sidebar:
    ...
    st.caption("LAcostWeb V13 | Clean Inputs")

# --- MAIN ---
st.title("LAcostWeb V13 🚀")
...
with col_sum2:
    # TAMAÑO DEL TOTAL AZUL REDUCIDO (De 2.5em a 1.8em)
    st.markdown(f"""
    ...
    if st.button("📧 Outlook Draft", type="primary", use_container_width=True):
        if not cust_name:
            ...
            subject = f"LAcostWeb V13: {cust_name} - Total {currency_sel} ${grand_total_price:,.2f}"
            body = f"""
