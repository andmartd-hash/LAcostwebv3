...
# Data: Countries (Actualizada con las dos opciones de Chile)
DB_COUNTRIES = pd.DataFrame([
    {"Country": "Argentina", "Currency_Code": "ARS", "Exchange_Rate": 1428.95},
    {"Country": "Brazil", "Currency_Code": "BRL", "Exchange_Rate": 5.34},
    {"Country": "Chile", "Currency_Code": "CLP", "Exchange_Rate": 934.70},
    {"Country": "Chile", "Currency_Code": "CLF", "Exchange_Rate": 0.02358}, # Nueva opción
    {"Country": "Colombia", "Currency_Code": "COP", "Exchange_Rate": 3775.22},
    {"Country": "Ecuador", "Currency_Code": "USD", "Exchange_Rate": 1.0},
    {"Country": "Peru", "Currency_Code": "PEN", "Exchange_Rate": 3.37},
    {"Country": "Mexico", "Currency_Code": "MXN", "Exchange_Rate": 18.42},
    {"Country": "Uruguay", "Currency_Code": "UYU", "Exchange_Rate": 39.73},
    {"Country": "Venezuela", "Currency_Code": "VES", "Exchange_Rate": 235.28}
])
...
def get_exchange_rate(country, currency_code):
    if currency_code == "USD":
        return 1.0
    
    # Buscar tasa específica para esa moneda y país
...
st.set_page_config(page_title="IBM Pricing Tool V4", layout="wide", page_icon="🌎")

st.title("🌎 IBM Services Pricing Tool V4")
st.caption("Includes Specific Chile Logic (CLP/CLF) | Multi-line | Global Risk")
...
with st.container():
    ...
    with c4:
        ...
        # Buscamos la tasa exacta basada en la selección (CLP, CLF, o la que sea)
        fx_rate = get_exchange_rate(country_sel, currency_sel)
        st.metric(f"Rate ({currency_sel})", f"{fx_rate:,.4f}")
...
if st.button("📧 Generate Email Draft"):
    if not cust_name:
        ...
        subject = f"Proposal: {cust_name} - Total {currency_sel} ${grand_total_price:,.2f}"
        body = f"""
