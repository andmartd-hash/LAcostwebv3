...
# ==========================================
st.set_page_config(page_title="LAcostWeb V9", layout="wide", page_icon="🏢")
...
st.markdown("""
    ...
    .css-1r6slb0 { border: 1px solid #e0e0e0; padding: 10px; border-radius: 5px; background: white;}
    h1 { color: #0F62FE; }
    </style>
...
with st.sidebar:
    ...
    # METRICAS CONDICIONALES
    
    # Solo mostrar Tasa de Cambio si NO es USD
    if currency_sel != "USD":
        st.metric("Exchange Rate", f"{fx_rate:,.2f}")
    
    st.metric("Risk Applied", f"{risk_val:.1%}")
    
    # Eliminada métrica "Local Logic/Active" por solicitud
    
    st.markdown("---")
    st.caption("LAcostWeb V9 | Internal Use Only")

# --- MAIN AREA ---
st.title("LAcostWeb V9 🚀")

# 1. SECCIÓN CLIENTE + CONTRATO (Todo a la derecha)
with st.container():
    st.subheader("👤 Customer & Contract Info")
    
    # Fila 1: Datos Cliente
    c_cust1, c_cust2 = st.columns([3, 1])
    with c_cust1:
        cust_name = st.text_input("Customer Name", placeholder="Enter Client Name...")
    with c_cust2:
        cust_num = st.text_input("Customer Number", placeholder="ID#")
    
    # Fila 2: Fechas (Quote y Contract)
    c_date1, c_date2, c_date3 = st.columns(3)
    with c_date1:
        quote_date = st.date_input("Quote Date", value=date.today(), disabled=True)
    with c_date2:
        # Movidos desde Sidebar
...
with col_sum2:
    ...
    if st.button("📧 Open in Outlook (Draft)", type="primary"):
        if not cust_name:
            ...
            subject = f"LAcostWeb V9: {cust_name} - Total {currency_sel} ${grand_total_price:,.2f}"
            body = f"""
LAcostWeb V9 Proposal
---------------------
