...
def add_line():
    st.session_state.lines.append({
        ...
        "unit_cost": 0.0, 
        "gp_percent": 40 # DEFAULT GP AHORA ES 40%
    })
...
with st.sidebar:
    ...
    st.caption("LAcostWeb V12 | Manual GP")

# --- MAIN ---
st.title("LAcostWeb V12 🚀")
...
for i, line in enumerate(st.session_state.lines):
    ...
    with st.expander(f"🔹 {i+1}. {current_offering_name}", expanded=True):
        ...
        with r2_c6: 
            # GP COMO TEXTO MANUAL (Sin botones)
            # Default value is taken from the line dictionary (40)
            gp_txt = st.text_input("GP %", value=str(line.get("gp_percent", 40)), key=f"gp_{i}")
            
            # Validación simple para convertir texto a número
            try:
                gp_percent_val = float(gp_txt)
            except ValueError:
                gp_percent_val = 0.0
            
            # Actualizamos el estado para que recuerde el valor si recarga
...
with col_sum2:
    ...
    if st.button("📧 Outlook Draft", type="primary", use_container_width=True):
        if not cust_name:
            ...
            subject = f"LAcostWeb V12: {cust_name} - Total {currency_sel} ${grand_total_price:,.2f}"
            body = f"""
